...
describe('getFileCommitDate', () => {
	...
	test('returns the newest commit date', () => {
		...
		expectCommitDateToEqual(getFileCommitDate(getFilePath(file), 'newest'), lastCommitDate);
	});
	...
	test('returns the initial commit date for a file never updated', () => {
		...
		expectCommitDateToEqual(getFileCommitDate(getFilePath(file)), commitDate);
	});
	...
	test('throws when failing to retrieve the git history for a file', () => {
		expect(() => getFileCommitDate(getFilePath('../not-a-starlight-test-repo/test.md'))).toThrow(
	...
	test('throws when trying to get the history of a non-existing or untracked file', () => {
		...
		expect(() => getFileCommitDate(getFilePath('untracked.md'))).toThrow(expectedError);
	});
});
...
type CommitDate = ReturnType<typeof getFileCommitDate>;
